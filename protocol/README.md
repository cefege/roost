...
| Worker local door | `127.0.0.1:4104`; `GET /api/local-bootstrap`; WS `/ws/local-terminal` with subprotocol `roost-local-terminal`; WS `/ws/local-attachment-transfer` with subprotocol `roost-local-attachment-transfer-v1` | Loopback only; exact coordinator-issued direct grant and descriptor. | `packages/protocol/src/local-ui-door.ts`; `apps/worker/src/local-door/local-ui-server.ts` | [`spec/direct-terminal.md`](spec/direct-terminal.md), [`spec/attachments.md`](spec/attachments.md) |

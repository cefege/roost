...
| **DEFERRED** | Automatic OMP conversation restoration | No resume command or restore setting ships in this slice. Involuntary-loss restoration remains disabled until the separate POSIX real-stack qualification proves adoption sends zero input, loss sends one official command, opaque shell metacharacters remain one argument, and rejected or ambiguous writes are never retried. | `ARCHITECTURE.md` |

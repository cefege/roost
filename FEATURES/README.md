...
| **SHIPPED** | Volatile agent status reads and waits | Dashboard-authorized Get/List/Wait RPCs and `roost api agent-status` / `agents` / `agent-wait` expose current worker-observed state and event-driven exact-occupant waits through a stable PID-free surface. Waits are bounded, never poll terminal output, and return nonzero unless matched; screen and identityless legacy rows remain unpromptable, and Roost owns no agent process, conversation, transcript, tool call, or approval model. | `apps/shared/src/wire/agent-status.ts`, `apps/coord/src/agent-status-hub.ts`, `apps/coord/src/connect/handlers-agent-status.ts`, `apps/roost-cli/src/api-agent-status.ts` |

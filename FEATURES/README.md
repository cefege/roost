...
| **SHIPPED** | Volatile agent status reads | Dashboard-authorized Get/List RPCs and `roost api agent-status` / `agents` expose current worker-observed state through a stable PID-free JSON projection. Epoch, occupant, and source are fencing observations only; screen and identityless legacy rows remain unpromptable, and Roost owns no agent process, conversation, transcript, tool call, or approval model. | `apps/shared/src/wire/agent-status.ts`, `apps/coord/src/connect/handlers-agent-status.ts`, `apps/roost-cli/src/api-agent-status.ts` |

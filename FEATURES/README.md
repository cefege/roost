...
| **SHIPPED** | Portable browser-local pane layouts | Copy/download and previewed explicit import use strict `LayoutDocumentV1` with deterministic preorder leaf/slot keys. Apply revalidates the current folder's live sessions, mints all runtime IDs afresh, appends new sessions to the focused pane, and publishes one browser-local commit; invalid documents are zero-mutation and open tabs are not live-synchronized. | `apps/shared/src/layout-document.ts`, `apps/web/src/store/paneLayoutDocument.ts`, `apps/web/src/lib/layoutDocumentControls.ts`, `smoke/terminal/layout-document.spec.ts` |

...
| **SHIPPED** | Dashboard scope isolation | Coordinator boot creates and validates the single local tenant, then server-resolved account-device membership scopes RPC, Sync, workers, and resources. Browser switches only after server confirmation, clear prior scoped state, and fence stale generations. | `apps/coord/src/self-hosted-tenant.ts`, `apps/coord/src/connect/dashboard-authorization.ts`, `apps/web/src/store/dashboard-selection.ts` |

...
- Assistant replies now stream into the transcript as they are generated instead of appearing whole at the end of the turn. The guest folds the host's incremental `message_update` deltas with `applyAssistantDelta` and re-renders per delta; the `message_end` snapshot still has the last word. Requires a host speaking collab protocol 4.
- The offline mock host (`bun run collab:mock-host`) replays its scripted turn as deltas, so the demo exercises the same streaming path as a live session.

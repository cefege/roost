...
/** Streamed in fragments; the concatenation is the committed block's text. */
const TURN_THINKING = ["Guest wants a live check. ", "I'll run the reconnect suite once and summarize the result."];
const TURN_TEXT = ["Kicking off a live reconnect probe ", "— one suite run, then a verdict."];
const TURN_CLOSE = ["Probe passed: 3 reconnects, ", "0 duplicate entries after resync. The reconnect path holds."];

const update = (delta: AssistantDelta): ScriptedStep => ({ kind: "event", event: { type: "message_update", delta } });
...
export function makeScriptedTurn(seq: number, parentId: string | null): ScriptedStep[] {
	...
	const a1Final: AssistantMessage = {
		role: "assistant",
		content: [
			{ type: "thinking", thinking: TURN_THINKING.join("") },
			{ type: "text", text: TURN_TEXT.join("") },
			toolCall,
		],
		model: fixtureModel.id,
	...
	const a2Final: AssistantMessage = {
		role: "assistant",
		content: [{ type: "text", text: TURN_CLOSE.join("") }],
		model: fixtureModel.id,
	...
	return [
		...
		update({ d: "thinking_start", i: 0 }),
		...TURN_THINKING.map(delta => update({ d: "thinking_delta", i: 0, delta })),
		update({ d: "text_start", i: 1 }),
		...TURN_TEXT.map(delta => update({ d: "text_delta", i: 1, delta })),
		update({ d: "toolcall_start", i: 2, id: toolCall.id, name: toolCall.name, intent: toolCall.intent }),
		update({ d: "toolcall_end", i: 2, toolCall }),
		...
		{ kind: "event", event: { type: "message_start", message: partial([]) } },
		update({ d: "text_start", i: 0 }),
		...TURN_CLOSE.map(delta => update({ d: "text_delta", i: 0, delta })),

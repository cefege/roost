...
import { GuestClient, type GuestSnapshot, type SessionActions } from "./lib/client";
...
function Session({ client, onLeave, onRejoin }: SessionProps): ReactNode {
	...
	const title = snapshot.header?.title ?? snapshot.state?.sessionName ?? "session";
...
export interface SessionSurfaceProps {
	...
	/** Optional host navigation; standalone App supplies the existing leave behavior. */
	onLeave?(): void;
	/** Optional host navigation; standalone App supplies the existing rejoin behavior. */
	onRejoin?(): void;
}
...
/**
 * Transport-neutral session presentation. The embedding host owns the snapshot
 * and actions; this component never subscribes to a client or opens a socket.
 */
		...
		<div className="sh-app">
			...
			<main className="sh-main">
				<section className="sh-content" data-rail={railOpen ? "true" : "false"}>
					<ActivityStrip todos={snap.todos} goal={snap.goal} activity={snap.activity} />
			...
			<Composer client={actions} snapshot={snap} />
			{drawerAgent && (
				<>

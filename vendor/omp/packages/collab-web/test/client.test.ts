...
describe("GuestClient frame apply", () => {
	...
	it("folds message_update deltas onto the stream ghost, one visible step per delta", () => {
		const client = liveClient();
		client.applyFrameForTest({ t: "event", event: { type: "message_start", message: assistantMessage("") } });
		const seen: string[] = [];
		for (const delta of ["He", "llo", " world"]) {
			client.applyFrameForTest({
				t: "event",
				event: { type: "message_update", delta: { d: "text_delta", i: 0, delta } },
			});
			const block = client.getSnapshot().stream?.content[0];
			seen.push(block?.type === "text" ? block.text : "<missing>");
		}
		expect(seen).toEqual(["He", "Hello", "Hello world"]);
		expect(client.getSnapshot().streamDone).toBe(false);
	});

	it("drops deltas that arrive with no message_start base", () => {
		const client = liveClient();
		client.applyFrameForTest({
			t: "event",
			event: { type: "message_update", delta: { d: "text_delta", i: 0, delta: "orphan" } },
		});
	...
	it("message_end keeps the ghost until the matching entry lands", () => {
		...
		client.applyFrameForTest({
			t: "event",
			event: { type: "message_update", delta: { d: "text_delta", i: 0, delta: "hello" } },
		});

...
- Bumped `COLLAB_PROTO` to `4`. The `message_update` agent event now carries an incremental `AssistantDelta` (`{ delta }`) instead of the full accumulating `WireMessage` (`{ message }`); `message_start` and `message_end` still carry the authoritative full message. Hosts previously re-serialized, re-encrypted, and re-sent the entire in-flight assistant message on every provider token, so a reply of length N cost O(N²) relay bytes and backlogged the socket until the turn ended. Guests speaking proto 3 are rejected at hello with the existing protocol-mismatch error.

### Added

- Added `AssistantDelta` and the pure reducer `applyAssistantDelta(message, delta)`, the shared fold both the TUI and browser guests apply to rebuild the in-flight assistant message from increments. Tolerant by design: a delta past the end of `content` extends it, and a delta whose block kind mismatches replaces the block rather than throwing, so a mid-turn join self-corrects at `message_end`.

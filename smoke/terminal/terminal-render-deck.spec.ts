...
import { pressPlatformShortcut, waitForStableCellFrames } from "./terminal-helpers.ts";
import type {
...
test("deck reveal preserves painted history and lands at the live bottom instantly", async ({ smokePage, stack }, testInfo) => {
  ...
  const revisited = await smokePage.evaluate((id) => ({
    painted: window.__smoke.paintedScrollback(id),
  }), sessionId);
  expect(revisited.painted.readerAnchor).not.toBeNull();
  expectPaintedRowsPreserved({ ...before.painted, rows: demandedSegment }, revisited.painted);

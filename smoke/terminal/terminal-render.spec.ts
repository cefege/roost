...
test("streaming sequence repair leaves an off-bottom reader fixed", async ({ smokePage, stack }, testInfo) => {
  ...
  const sample = () => smokePage.evaluate((id) => {
    ...
    return {
      ...
      atBottom: container.scrollTop >= container.scrollHeight - container.clientHeight,
      row: row?.textContent ?? "",
  ...
  await expect.poll(async () => {
    ...
    return !state.atBottom && state.row.includes("READERLINE-")
      && (await readTerminalStreamProbe(smokePage, sessionId)).browser.presentation?.reader_intent === "reading";
  }, { timeout: 10_000 }).toBe(true);
  // The echo-free shell makes the first post-arm delivery the controlled 1501 marker.
...
test.describe(() => {
  ...
  // Fixture setup consumes this budget, so it must be declared before smokePage resolves.

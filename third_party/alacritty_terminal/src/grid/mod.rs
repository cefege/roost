...
pub struct Grid<T> {
    ...
    /// Roost addition: history lines this grid has scrolled off the top,
    /// since it was created. See `ROOST-PATCHES.md`.
    ///
    /// Defaults on load so a recorded reference fixture written before the
    /// patch still deserializes.
    #[cfg_attr(feature = "serde", serde(default))]
    discarded_line_count: u64,
}
...
impl<T: GridCell + Default + PartialEq> Grid<T> {
    ...
    /// Grow the scrollback by up to `count` lines and report how many it
    /// actually took.
    ///
    /// Roost addition: the return value is what the caller needs in order to
    /// count evictions. The clamp below is the eviction decision — a request
    /// for `n` lines against a history with `m` slots gives back `m`, and the
    /// `n - m` that remain are the lines the grid cannot hold and the
    /// embedder's history origin has to move past.
    fn increase_scroll_limit(&mut self, count: usize) -> usize {
        let granted = min(count, self.max_scroll_limit - self.history_size());
        if granted != 0 {
            self.raw.initialize(granted, self.columns);
        }
        granted
    }
        ...
        D: PartialEq,
    {
        ...
        if region.start == 0 {
            ...
            // has to move its origin past it. See `ROOST-PATCHES.md`.
            let granted = self.increase_scroll_limit(positions);
            self.discarded_line_count += (positions - granted) as u64;
...
impl<T> Grid<T> {
    ...
    /// Roost addition: history lines this grid has scrolled off the top since
    /// it was created.
    ///
    /// The lines still addressable are
    /// `discarded_line_count() .. discarded_line_count() + history_size()`.
    /// A line outside that window is gone, not merely scrolled, and only this
    /// count distinguishes the two. See `ROOST-PATCHES.md`.
    #[inline]
    pub fn discarded_line_count(&self) -> u64 {
        self.discarded_line_count
    }

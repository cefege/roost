...
impl<T> Term<T> {
    /// Roost addition: history lines the active grid has scrolled off the top
    /// since this terminal was created.
    ///
    /// See `third_party/alacritty_terminal/ROOST-PATCHES.md`.
    #[inline]
    pub fn discarded_line_count(&self) -> u64 {
        self.grid.discarded_line_count()
    }
}

...
import { composeStandaloneIntegration } from "../apps/worker/src/agents/standalone-integration.ts";
import { AGENT_INTEGRATION_ASSET_SPECS } from "../apps/worker/src/agents/integration-assets.ts";
...
const AGENT_INTEGRATIONS_OUT = "apps/worker/src/agents/integration-assets.generated.ts";
...
async function generate(): Promise<void> {
  ...
  const reportTransport = readFileSync(
    "apps/worker/src/agents/report-transport.ts",
    "utf8",
  );
  const agentIntegrations = AGENT_INTEGRATION_ASSET_SPECS.map((spec) => ({
    ...
    content: composeStandaloneIntegration(
      readFileSync(
        join("apps/worker/src/agents", spec.sourcePath),
        "utf8",
      ),
      reportTransport,
    ),

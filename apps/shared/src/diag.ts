...
export type SignalKind =
  ...
  | "terminal.capture_started"   // an operator armed opt-in terminal incident recording on one session; kv names recording ID, lease expiry and which layers acknowledged. Consent for sensitive capture was granted HERE — the audit trail for why raw terminal bytes exist on disk at all
  | "terminal.capture_saved"     // one incident bundle was frozen and written; kv names capture ID, byte length, per-layer coverage and the trigger reason. cooldownKey is the capture ID so a distinct saved file is never coalesced away by the generic per-kind cooldown
  | "terminal.capture_failed"    // a capture could not complete; kv names the fixed error code and which layers were unavailable. The browser still holds its frozen evidence for retry, so this is not evidence loss
  | "terminal.capture_stopped"   // recording released by the owner; in-memory trace state freed, saved files retained
  | "terminal.capture_expired"   // a lease passed its server-time expiry without renewal; recorders disarmed themselves and a page reload must START again rather than silently resume
  | "terminal.history_conflict"  // the painted scrollback violated a structural invariant: a duplicate or out-of-order ABSOLUTE row index, or DOM history whose identity/order disagrees with the committed painted model at the same completed boundary. This is the duplicated-footer class; kv names stream/epoch/reason identity and counts only
  | "terminal.emission_conflict" // a fresh viewport-only core scan disagreed with the worker's own emitted-frame fold at one emission's exact generation and sequence — the grid the core holds is not the grid the worker shipped. kv names the differing field plus row/column, never the text
  | "voice.ws_failed"

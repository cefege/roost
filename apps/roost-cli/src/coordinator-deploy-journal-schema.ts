// Coordinator deploy journal schema, confinement, and fail-closed parsing.
// coordinator-deploy-journal.ts exposes this API and owns phase writes.
// Service-definition parsing and POSIX path rules provide recovery proof.
...
export const COORDINATOR_DEPLOY_JOURNAL_SCHEMA_VERSION = 3 as const;
...
export interface CoordinatorWorkerKeeperPlanV1 {
  fingerprint: string;
...
export interface CoordinatorDeployJournalV2 {
  ...
  workerKeeperPlans: CoordinatorWorkerKeeperPlanV1[];
...
function parseWorkerKeeperPlans(
  value: unknown,
  targetWorkerFingerprints: readonly string[],
): CoordinatorWorkerKeeperPlanV1[] {
  if (!Array.isArray(value) || value.length !== targetWorkerFingerprints.length) {
    throw new Error("workerKeeperPlans must cover every target worker");
  }
  return value.map((candidate, index) => {
    const record = posixJournalObjectValue(
      candidate,
      "workerKeeperPlans entry must be an object",
    );
    const fingerprint = record.fingerprint;
    if (fingerprint !== targetWorkerFingerprints[index]) {
      throw new Error("workerKeeperPlans must match the canonical worker order");
    }
    const keeperUpdate = JournaledKeeperUpdateV1Schema.parse(record.keeperUpdate);
    return {
      fingerprint,
      keeperUpdate,
    };
...
export function parseCoordinatorDeployJournal(
  ...
  const workerKeeperPlans = parseWorkerKeeperPlans(
    fields.workerKeeperPlans,
    targetWorkerFingerprints,
  );
  const admissionRecordedAtMs = fields.admissionRecordedAtMs;
  if (!Number.isSafeInteger(admissionRecordedAtMs)
    || (admissionRecordedAtMs as number) <= 0) {
    throw new Error("admissionRecordedAtMs must be a positive integer timestamp");
  }
  ...
  return {
    ...
    rolloutId,
    targetWorkerFingerprints,
    admissionRecordedAtMs: admissionRecordedAtMs as number,
    workerKeeperPlans,

// `roost join` — run on a NEW Mac (invoked by join.sh) to install + start the
// local worker and register it with the coordinator. The pull counterpart to
// `roost deploy`: no SSH, no rsync — the code is already cloned here, so this
// just runs the local-install path. Reads ROOST_COORDINATOR_URL +
...
export async function join(_args: string[]): Promise<void> {
  ...
  await _deployLocal("this Mac");

  console.log("");
  console.log("Joined. This Mac should appear in Settings → Machines within a few seconds.");

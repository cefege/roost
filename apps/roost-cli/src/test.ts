...
// Profiles keep the hermetic unit, terminal and upgrade-continuity coverage and
// the live-tailnet API canary behind distinct prerequisites.

import { spawn } from "bun";

const PROFILES = ["unit", "worker", "terminal", "upgrade", "live-api", "all"] as const;
...
async function runUpgrade(): Promise<void> {
  try {
    // Same smoke-enabled bundle as the terminal tier: the upgrade proof reads
    // PTY markers back through window.__smoke after the deploy.
    await run("web build", [process.execPath, "run", "--cwd", "apps/web", "build"], {
      VITE_ROOST_SMOKE: "1",
    });
    // Real embeds here rather than stubs, because this run also writes the
    // keeper implementation digest that update admission compares against the
    // digest each staged release generates for itself.
    await run("web embed", [process.execPath, "scripts/gen-embed.ts"]);
    await run(
      "upgrade",
      [
        process.execPath, PLAYWRIGHT_CLI, "test",
        "--config=smoke/upgrade/playwright.config.ts",
      ],
      { ROOST_TEST_BUN: process.execPath },
    );
  } finally {

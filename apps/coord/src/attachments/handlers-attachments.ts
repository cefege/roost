...
import { getWorkerHubSocket, sendAttachmentChunk } from "../workers/worker-service.ts";
import {
  createPendingRpc,
  createPendingRpcWithId,
  rejectPendingRpcUnavailable,
} from "../router/pending-rpcs.ts";
import { sendBrowserCmd, requireSessionWorkerSocket } from "../rpc/router-helpers.ts";
import type { ConnectDeps } from "../rpc/router.ts";

async function requireWorkerHubSocket(
  deps: ConnectDeps,
  workerFp: string,
) {
...
export function makeAttachmentHandlers(
  ...
  return {
    async filesRead(req, ctx) {
      ...
      const { workerFp, sock } = await requireWorkerHubSocket(deps, req.workerFp);

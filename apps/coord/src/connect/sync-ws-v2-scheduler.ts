...
// the SAME application ACK window as the v1 send path — the window bookkeeping
// and every close path are injected from sync-ws-v1-delivery.ts.
//
// Split out of sync-ws-handler.ts. Frame ORDER is load-bearing: we may step
// past an ineligible fenced cell to reach its opened event, but two eligible
// items from the same domain are never reordered.
...
export function makeSyncV2Scheduler(deps: SyncV2SchedulerDeps) {
  ...
  return { sendV2ControlFrame, resetV2Domain, scheduleV2, enqueueV2Frame };
}

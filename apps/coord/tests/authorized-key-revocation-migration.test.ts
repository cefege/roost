...
describe("authorized key revocation migration", () => {
  test("upgrades a pre-0018 database and enforces both trigger boundaries", async () => {
    ...
    sqlite.exec("BEGIN");
    sqlite.prepare("INSERT INTO marker (value) VALUES (?)").run("must-rollback");
    ...
    expect(sqlite.prepare("SELECT value FROM marker").all()).toEqual([]);
    ...
    expect(sqlite.prepare("SELECT token FROM bootstrap_tokens").all()).toEqual([]);
    sqlite.close();
  });
  ...
  test("a failed script rolls back and is not recorded", async () => {
    ...
    expect(sqlite.prepare("SELECT name FROM _migrations WHERE name = ?").get("0018_broken")).toBeNull();
    expect(() => sqlite.prepare("SELECT * FROM should_rollback").all()).toThrow();
    sqlite.close();
  });
});

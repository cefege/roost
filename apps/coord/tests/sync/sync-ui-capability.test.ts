...
// A read-only socket keeps its UI-bus subscription for legacy delivered counts,
// but receives neither retained/live reports nor legacy/apply commands.
...
import { uiBus } from "../../src/events/buses.ts";
...
test("read-only Sync counts as subscribed but receives no seeded or live UI frames", async () => {
  ...
  const beforeSubscribers = uiBus.subscriberCount;
  ...
  expect(uiBus.subscriberCount).toBe(beforeSubscribers + 2);
  ...
  uiBus.publish({ kind: "command", targetTabId: "", command });
  uiBus.publish({
    kind: "apply",
  ...
  expect(uiBus.subscriberCount).toBe(beforeSubscribers);
});

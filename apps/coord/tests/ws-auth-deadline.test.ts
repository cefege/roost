...
} from "../src/connect/ws-auth-deadline.ts";
...
class FakeClock implements WsDeadlineClock {
  nowMs = 0;
  maxDelayMs = 50;
  nextId = 1;
...
describe("WebSocket authentication deadline", () => {
  ...
  test("re-arms deadlines beyond the platform timer maximum", () => {
    const clock = new FakeClock();
    const closes: number[] = [];
    scheduleWsAuthDeadline({ close: (code) => { closes.push(code ?? 0); } }, 120, clock);
    clock.advance(50);
    expect(closes).toEqual([]);
    clock.advance(50);
    expect(closes).toEqual([]);
  ...
  test("ordinary close cleanup cancels the active timer handle", () => {
    const clock = new FakeClock();
    const closes: number[] = [];
    const handle = scheduleWsAuthDeadline({ close: (code) => { closes.push(code ?? 0); } }, 100, clock);

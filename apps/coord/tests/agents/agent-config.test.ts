...
import { getAgentConfig, setAgentConfig } from "../../src/agents/agent-config.ts";
...
beforeAll(async () => {
  ...
  db = opened.db;
  dashboardId = ensureSelfHostedTenant(opened.sqlite, {
...
describe("agent-config", () => {
  ...
  test("built-in selection round-trips", async () => {
    await setAgentConfig(db, dashboardId, { selected: "codex", customCommand: "", autoLaunch: false });
    expect((await getAgentConfig(db)).selected).toBe("codex");
  });

  test("custom command round-trips", async () => {
    await setAgentConfig(db, dashboardId, { selected: "custom", customCommand: "aider", autoLaunch: false });
  ...
  test("empty selected falls back to OMP", async () => {
    await setAgentConfig(db, dashboardId, { selected: "", customCommand: "", autoLaunch: false });
    expect((await getAgentConfig(db)).selected).toBe("omp");
  });

  test("auto-launch toggle round-trips", async () => {
    await setAgentConfig(db, dashboardId, { selected: "omp", customCommand: "", autoLaunch: true });
    expect((await getAgentConfig(db)).autoLaunch).toBe(true);
    await setAgentConfig(db, dashboardId, { selected: "omp", customCommand: "", autoLaunch: false });

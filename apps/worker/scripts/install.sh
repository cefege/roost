...
# error for one it never loaded: the agent simply stays absent from `launchctl
# list` until an operator notices the machine went quiet. A redirect straight
# onto $PLIST truncates the installed agent before the first byte of the
# replacement lands, so any interruption leaves an unloadable stub and no way
# back. Stage beside the target, prove it parses, then rename over it.
plist_is_complete() {

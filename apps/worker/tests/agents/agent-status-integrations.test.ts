...
async function importIntegration(agent: "omp" | "pi"): Promise<IntegrationModule> {
  const url = new URL(
    `../../src/agents/integrations/${agent}/roost-agent-state.ts?test=${Date.now()}-${Math.random()}`,
    import.meta.url,
  );
...
async function importOmpReferenceIntegration(): Promise<IntegrationModule> {
  const url = new URL(
    `../../src/agents/integrations/omp/roost-agent-reference.ts?test=${Date.now()}-${Math.random()}`,
    import.meta.url,
  );
  return await import(url.href) as unknown as IntegrationModule;
}

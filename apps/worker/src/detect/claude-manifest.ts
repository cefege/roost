// Claude Code screen-detection rules. Ported verbatim from herdr's
// src/detect/manifests/claude.toml (id="claude", version 2026.06.10.3).
// Matched by detect/manifest-engine.ts::evaluate against the rendered grid.
// Each rule = a region + gate; highest priority match wins. visible_blocker
// rules are the high-confidence "claude is parked waiting on you" signal that
// the claude --settings hooks miss (only permission-request fires a hook).
// To re-sync: diff against herdr claude.toml; this is data, not logic.

import type { Rule } from "./manifest-engine.ts";

export const CLAUDE_RULES: Rule[] = [
  {
    id: "osc_title_working", state: "working", priority: 1100,
    region: "osc_title", visible_working: true,
    regex: ['^[\\x{2800}-\\x{28FF}] '], // braille spinner in the window title
  },
  {
  ...
  },
  {
    ...
    regex: ['^\\x{2733} '], // ✳ idle glyph in the window title
  },
  {

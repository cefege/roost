...
/// device may claim the next pending task without naming one -- so it is
/// derived, and must be unique: sequence within, boot identity across.
pub fn new_task_id(process_epoch: &str, boot_ms: i64) -> String {
    ...
    // The version and variant nibbles are what every consumer actually checks.
    bytes[6] = (bytes[6] & 0x0f) | 0x40;
    bytes[8] = (bytes[8] & 0x3f) | 0x80;
    let hex = hex::encode(bytes);
    let tail = &hex[20..];
    format!("{}-{}-{}-{}-{tail}", &hex[..8], &hex[8..12], &hex[12..16], &hex[16..20])
}

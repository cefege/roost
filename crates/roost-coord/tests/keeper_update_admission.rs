...
use keeper_support::{
    EPOCH, Observed, connect_observer, digest, enroll_device, journal, journaled, maintenance,
    observed_by, prepare,
};
use roost_coord::deploy::keeper_update::{
    MAINTENANCE_ACTION, KeeperUpdateAction, handle_workers_prepare_keeper_update,
};
use roost_protocol::wire::coord_worker::CoordWorkerDownstream;
use serde_json::json;
use support::{WORKER_FP, WorkersFixture, device_caller};
...
async fn an_authorized_maintenance_may_cross_a_live_session() {
    ...
    let response = handle_workers_prepare_keeper_update(
        ...
        prepare(
            WORKER_FP,
            roost_proto::WorkersPrepareKeeperUpdateRequest {
                force_live: true,
                ..maintenance()
            },
        ),
    ...
    match &observed.frames[0] {
        CoordWorkerDownstream::KeeperUpdatePrepare(body) => {
            ...
            assert_eq!(body.coordinator_open_session_ids, vec![support::SESSION_ID]);
        }
        other => panic!("a keeper update goes out as its own frame, not {other:?}"),
    }
}

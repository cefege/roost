...
use roost_coord::diagnostics::rpc_transcription::{
    handle_transcription_get_config, handle_transcription_grant_token, handle_transcription_set_config,
...
async fn an_absent_key_is_left_alone_and_a_blank_one_clears_it() {
    ...
    let after_language_only = handle_transcription_set_config(
        &fixture.core,
        &caller,
        set_request(None, "de"),
    )
    ...
    let after_blank = handle_transcription_set_config(
        &fixture.core,
        &caller,
        set_request(Some("   "), "de"),
    )
...
async fn the_handoff_is_the_configured_key_itself_and_never_a_temporary_grant() {
    ...
    assert_eq!(grant.access_token, STORED_KEY, "the browser connects with it");
    assert_eq!(
        grant.expires_in, 0,
...
async fn none_of_the_four_methods_answer_a_caller_that_is_not_a_browser() {
    ...
    for (index, refusal) in refusals.iter().enumerate() {
        ...
        assert!(
            refusal.response_headers().contains_key(roost_coord::auth::principal::AUTH_LAYER_HEADER),

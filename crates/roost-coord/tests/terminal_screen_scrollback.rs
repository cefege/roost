//! The scrollback reads: the window a page names, the answer a page may carry,
//! and the cancel that has to retire a search it beat.
//!
...
fn a_page_window_is_the_max_rows_immediately_before_the_exclusive_end() {
    ...
    assert!(window.is_served_by(400, 500));
    assert!(
        !window.is_served_by(401, 500),
        "a page that starts late does not cover the window"
    );
    assert!(
        !window.is_served_by(400, 499),
        "a page that ends early does not cover the window"
    );
}

...
async fn a_refused_pair_confirmation_is_recorded_and_an_accepted_one_is_not() {
    ...
    let local = ListenerTrust::DirectLoopback;
    // `record_request` consumes the record, so each binding is a local the hook
    // mutates rather than a value an assertion borrows.
    let mut accepted = on("PairConfirm", 200, Some(CALLER), local);
    let mut refused = on("PairConfirm", 200, Some(CALLER), local).pair_confirmation_failed(true);

...
11. **L11-RECURRING-FAILURE-INDEX — grep this BEFORE writing any
    ...
    | "terminal scrollback jumps around / view lurches while scrolling up / snaps back to bottom after a tab switch / won't stay at the bottom" | n/a — fixed 2026-07-25 | add another follow/pin flag to `CellTerminal.tsx` (it had eleven: a 12-frame reveal re-pin burst, a NaN pin-echo suppressor, touch slop + momentum grace, a spring jump animation) — they fight each other and each new one adds a new race | **Two things only.** (1) `CellGridRenderer.sizeBlock()` sets an EXACT inline `contain-intrinsic-size` (rows × 1.2em) on every `.cell-block`, so a skipped `content-visibility` block's placeholder equals its real height and revealing it reflows nothing — a flat estimate overstates every partial block and IS the lurch. (2) The follow rule is `stick` + `lastTop` + one scroll listener: `atBottom() → stick=true`, else `scrollTop decreased → stick=false`; the pin runs in `scheduleFlush`'s rAF. If the view misbehaves, `scrollHeight` is lying — fix that, do not add a guard. |

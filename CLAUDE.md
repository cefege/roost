...
11. **L11-RECURRING-FAILURE-INDEX — grep this BEFORE writing any
    ...
    | "tab switch / reveal waits on history before the live bottom is readable / deep sessions reveal slower than shallow ones" | n/a — fixed 2026-08-04 (measured: a 2000-row catch-up frame is 324–516 KiB of proto, 38 ms of blocked worker event loop, and ~300 ms of `renderFull` scrollback DOM built BEFORE `renderViewport()`) | bridge the claim snapshot's tail back to the viewer's held boundary (`_claimTailRows`, cap 2000) — that payload is atomic with the viewport on the broadcast Sync WS, so nothing paints until all of it lands, and every cost scales with how long the pane was parked; equally wrong: shrinking or racing history away, or reordering `renderFull` to paint the viewport first (breaks the single `_pinToBottom` writer, row 122) | **The claim snapshot is ALWAYS `SB_SNAPSHOT_TAIL_ROWS` — constant-size, ~40 KiB, whatever the depth or the gap.** Collapsing to the tail is the CORRECT answer (`apply()`'s `windowUnreachable` pins the literal bottom) because `scrollbackBackfill.ts` now refills the whole `MAX_HELD_SCROLLBACK_ROWS` window in ONE `BACKFILL_CONCURRENCY`-wide wave off the broadcast feed, triggered by the reveal itself (`onReveal`, since `held_cell_seq` may skip the snapshot entirely) and gated on `active()` so a parked pane never pulls; the worker's read yields the event loop every `SCROLLBACK_CELLS_SLICE_ROWS` and ABORTS with an rpc-error rather than return an index-shifted hole. `held_scrollback_total` is deleted end-to-end (proto field 6 reserved). Regression: `terminal.spec.ts` "deep-history attach/reveal…" (markerMax 9000 + `atBottom` + `rowCount < 500` within 1 s, then `rowCount > 1800` with NO scroll), `apps/worker/tests/scrollback-cells-backfill.test.ts` "claim snapshot is always the constant tail". |

#!/usr/bin/env bash
# Roost one-click installer. Run on a Mac to go from nothing → running:
...
# 0. macOS only (LaunchAgents + tailscale cert assume darwin).
[ "$(uname -s)" = "Darwin" ] || die "Roost installs on macOS only (found $(uname -s))."
...
#    need Tailscale). The open-source tailscaled (brew) needs NO System Settings
#    approval. Was a hard die() that stranded users before the guided gate.
...
if ! command -v tailscale >/dev/null 2>&1; then
  ...
  say "WARN: Tailscale not running — quickstart will guide you (sudo tailscale up)."
fi
